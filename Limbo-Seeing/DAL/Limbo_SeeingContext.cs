...
namespace Limbo_Seeing.DAL
{
    class Limbo_SeeingContext : DbContext
    {
        ...
        public DbSet<Activiteiten> Activiteiten { get; set; }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Activiteiten>().HasOne(p => p.Gebruiker).WithMany(M => M.Activiteitens).HasForeignKey(F => F.Gebruiker_Id);
            modelBuilder.Entity<Activiteiten>().HasMany(M => M.Reseverings).WithOne(O => O.Activiteit).HasForeignKey(F => F.Activiteit_Id);
